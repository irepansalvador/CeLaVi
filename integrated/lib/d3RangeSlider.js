 ...
 */
function createD3RangeSlider (rangeMin, rangeMax, containerSelector, playButton, delta) {
    "use strict";
...
//						.style("margin-top", "80px");
    ...
    function frameTick() {
     ...
     //   var delta = Math.min(Math.ceil(rangeWidth / 10), Math.ceil(limitWidth / 100));
        var delta= sliderRange.end - sliderRange.begin ;
        // Check if playback has reached the end
        if (sliderRange.end + delta > rangeMax) {
            delta = rangeMax - sliderRange.end;
            stopPlaying();
        }
				console.log(sliderRange, delta);
